...
abstract: |
 Het belang van dit onderzoek is om te kijken hoe verschillende kiemgroenten reageren op licht verhoogde zoutconcentraties in   de groeibodem, dit is belangrijk omdat de zeespiegel stijgt en hierdoor de grond geleidelijk aan zouter wordt. We hebben 400   keer tuinkers en 400 keer alfalfa geplant en deze gevoed met verschillende oplopende zoutconcentraties. Hieruit blijkt  dat een oplopende zoutconcentratie een significant effect heeft op de groei van beide planten. Omdat de verzamelde data niet   volledig betrouwbaar is zal dit onderzoek opnieuw uitgevoerd moeten worden om met zekerheid te kunnen zeggen dat een zoutconcentratie effect heeft op de groei van kiemgroenten.
...
Opwarming van de aarde heeft veel negatieve invloeden op de wereld. De hitte, droogte en zout gehaltes in de grond, wijken steeds meer af van de optimale waarden voor planten. In 2021 had 33% van de geïrrigeerde landbouwgronden een te hoge zout concentratie in de bodem. Ook wordt er verwacht dat dit elk jaar met 10% gaat stijgen. De meeste planten die door ons gegeten worden zijn glycofyten. Dit zijn planten die niet tolerant zijn tegen een te hoge zout concentratie. Zout stress zal bij veel van deze planten impact hebben op de groei, ontkieming van zaadjes en algemene ontwikkeling van de plant. [@soil_salinity; @Salt_stress]. 

Zout kan op 2 manieren een negatieve invloed hebben in een plant. Wanneer de grond een hoge zout concentratie opneemt kan een plant veel minder water opnemen via de wortels, dit zal de groei van een plant op een negatieve manier beïnvloeden. Wanneer de plant toch een stuk gegroeid is kan het zijn dat er een te hoge concentratie van ionen in de plant zit. Deze ionen kunnen ervoor zorgen dat de membraan structuren van de plant stuk gaan en dit kan zelfs de fotosynthese van een plant negatief beïnvloeden, dit zal dan ook weer een negatieve impact hebben op de groei en ontwikkeling van de plant. Daarnaast zal dit het uiterlijk van de plant beïnvloeden, de blaadjes zullen niet geen groene kleur hebben en worden meer geelgekleurd. [@soil_salinity; @Salt_stress]. 
...
Er was weinig bekend over kiemgroentes, en hoe deze planten reageren op een verhoogde zoutconcentratie. Daarom is er onderzoek naar gedaan. 
...
Is er een verschil tussen deze 2 planten, en hoe wordt er omgegaan met de verschillende zoutconcentraties? 
...
Om de effectsterkte tussen de 2 duplo's te berekenen is cohen's D gebruikt. De effect sterkte wordt gebruikt om het verschil tussen groepen te kwantificeren. Een t-test zal weergeven of er een significant verschil is tussen 2 gemiddelden en de effectsterkte hoe groot dit verschil daadwerkelijk is. Dit is belangrijk voor het onderzoek. Omdat er een grote samplesize is, zal een statische test al snel erop wijzen dat er een significant verschil is. De effectsterkte zal juist weergeven hoe groot dit effect is. Voor de duplo's is gebruik gemaakt van cohen's D, deze wordt gebruikt voor de effectsterkte tussen de gemiddelden. Het resultaat van deze berekening geeft weer hoeveel standaarddeviaties er tussen de 2 gemiddelden ligt, een D van 0.3 betekend dus dat er 0.3 standaarddeviaties tussen de 2 gemiddelden ligt. [@effect_size; @cohensD]. 
...
Voor de effectsterkte is bij deze test Fisher 's eta2 berekend. Net zoals Cohen's D is dit om het verschil tussen groepen te kwantificeren. In dit geval kan het gebruikt worden als er meerdere gemiddelden zijn en 2 factoren. [@effect_size] 
...
# Resultaten
## Duplo's
Aan het einde van de datcollectie is de lengte, ontkieming, aantal blaadjes en de kleur bepaald van 800 plantjes verspreid over 2 soorten en 4 verschillende zout oplossingen. Omdat er met duplo's gewerkt is, is er eerst gekeken naar verschil tussen duplo's (figuur \@ref(fig:boxplotduplo)).  
...
```
Voor alle 4 testen geldt p < 0.05, dus er zit bij deze een significant verschil tussen de geteste duplo's.  
...
```
  ...
  Wat zichtbaar is uit deze grafiek is dat bij 0.09% de verdeling een stuk meer naar rechts licht dan bij de andere oplossingen. de verdeling van groei bij 0%, 0.03% en 0.06% lijkt allemaal wel in de buurt te liggen van elkaar. Alleen is de spreiding bij 0.06% een stuk minder breed.
...
data_set_plants %>%
    ...
    facet_wrap(~ oplossing)
```
Wat zichtbaar is in deze grafiek is dat het aantal blaadjes per plant niet veel veranderd wanneer de oplossing wordt veranderd.
